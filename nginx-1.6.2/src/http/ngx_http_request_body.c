...
//获取http包体，是一个异步方法，调用它只是说明要求nginx开始接收请求的包体，并不表示是否已经接收完，当接收完所有的包体内容后，post_handler指向的回调方法会被调用。
ngx_int_t
    ...
    ngx_http_client_body_handler_pt post_handler)
{
	...
	//此值为异步操作计数加1,当异步操作结束时应该调用ngx_http_finalize_request方法把引用计数减1
    r->main->count++;
