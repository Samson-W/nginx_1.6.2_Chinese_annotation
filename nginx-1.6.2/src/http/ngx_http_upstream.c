...
//创建upstream,仅创建了upstream结构体，其中的成员需要各个http模块自行设置 
ngx_int_t
...
//启动upstream机制,根据ngx_http_upstream_conf_t中的成员初始化upstream，同时开始连接上游服务器，以展开整个upstream处理流程
void
...
#endif
	//检查请求对应于客户端的连接，这个连接上的读事件如果在定时器中，把这个读事件从定时器中移除，请求的主要触发事件将以与上游服务器的连接为主
    if (c->read->timer_set) {
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
	...
	//向原始请求中的cleanup链表末尾添加一个新成员，这表示当请求结束时，一定会调用ngx_http_upstream_cleanup方法
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    ngx_http_upstream_t  *u;
	//由事件的data成员取得ngx_connection_t连接，是nginx与上游服务器间的连接
    c = ev->data;
	//由连接的data成员取得ngx_http_request_t结构体
    r = c->data;
	//由请求的upstream成员取得表示upstream机制的ngx_http_upstream_t结构
    u = r->upstream;
    ...
    if (ev->write) {
	//当nginx与上游服务器间tcp连接的可写事件被触发时，upstream的write_event_handler被调用
        u->write_event_handler(r, u);

    } else {
		//当nginx与上游服务器间tcp连接的可读事件被触发时，upstream的read_event_handler方法被调用
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    u->request_sent = 0;
	//连接若没有建立成功，把写事件添加到定时器中，超时时间是ngx_http_upstream_conf_t中的connect_timeout
    if (rc == NGX_AGAIN) {
...
ngx_http_upstream_send_request(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	//向上游服务器发送ngx_http_upstream_t结构体中的request_bufs链表
	...
	//表示还有请求未被发送，添加写事件到定时器中，防止发送请求超时
    if (rc == NGX_AGAIN) {
...
{
    ngx_connection_t  *c;
	//得到与上游服务器的connection结构
    c = u->peer.connection;

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "http upstream send request handler");
	//timeout为1时表示向上游服务器发送的请求已经超时
    if (c->write->timedout) {
		//进行错误重连策略决定
...
#endif
	//为1时表示上游服务器的响应要直接转发给客户端，而且此时nginx已经把响应包头转发给了客户端了
    if (u->header_sent) {
		//表明已经解析完全部了上游响应包头，不再应许向上游发送请求，修改写事件为什么都不做的回调
        u->write_event_handler = ngx_http_upstream_dummy_handler;
		//将写事件添加到epoll中
...
//处理回应的包头，将头部设置到发送给下游客户端的http响应包头中
static void
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	//接收响应头部的buffer缓冲区，若它的start成员指向空，说明缓冲区还没有分配内存，配置配置的buf size大小的内存
	...
	//处理已经解析出的头部，设置到ngx_http_request_t结构体的headers_out成员
    if (ngx_http_upstream_process_headers(r, u) != NGX_OK) {
        return;
    }
	...
	//是否实现了处理包体的input_filter方法，如果没有实现，则使用upstream定义的默认方法替代input_filter，若用户已经实现了input_filter方法，表示用户希望自己处理包体
...
//负责接收上游服务器的包体，同时调用http模块实现的input_filter方法处理包体
static void
...
ngx_http_upstream_process_non_buffered_downstream(ngx_http_request_t *r)
{
	...
	//调用方法向客户端发送响应包体，第2个参数是1，和upstream发送响应包体的不一样
    ...
    ngx_http_upstream_t *u)
{
	...
	//真正决定以固定内存块作为缓存时如何转发响应的，注意第2个参数是0
...
//这个默认的方法将会试图在buffer缓冲区中存放全部的响应包体 497
static ngx_int_t
ngx_http_upstream_non_buffered_filter(void *data, ssize_t bytes)
{
	//http模块的未实现input_filter时，input_filter_ctx指向请求的ngx_http_request_t
    ...
    u = r->upstream;
	//找到out_bufs链表的末尾，其中 cl指向ngx_chain_t元素的next成员，所以cl最后一定是NULL指针，而ll指向最后一个缓冲区的地址，它用来在后面在代码中向out_bufs链表添加新的缓冲区
