...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
	...
	//进行11个阶段的处理函数进行检查，检查到对应的配置项，即调用对应模块的handler
...
//yy 发送http头部 会首先调用所有的http过滤模块共同处理header out中定义的http响应头部，全部处理完后才会序列化为tcp字符流发送到客户端
ngx_int_t
