...
//连接建立成功且获取到ngx_connection_t结构体后，已经分配过内存池，并且在表示初始化了读写事件后的连接数
...
//添加读事件到epoll等事件驱动模块中，该事件对应的tcp连接上一旦出现可读事件(如接收到tcp连接另一端发送来的字符流)，就会回调该事件的handler方法，flags指定事件的驱动方式
ngx_int_t
...
//将写事件添加到事件驱动模块中。lowat表示只有当连接对应的套接字缓冲区中必须有lowat大小的可用空间时，事件收集器才能处理这个可写事件
ngx_int_t
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
	...
	//ngx_accept_mutex就是负载均衡锁，spin值为-1则是告诉nginx这把锁不可以使进程进入睡眼状态
