...
struct ngx_event_s {
	//事件相关的对象。通常data都是指向ngx_connection_t连接对象。开启文件异步io时，它可能会指向ngx_event_aio_t结构体
    void            *data;
	//为1表示事件是可写的。通常情况下，它表示对应的tcp连接目前状态是可写的，也就是连接处于可以发送网络包的状态
    unsigned         write:1;
	//为1时表示事件可发建立新的连接。通常情况下，在ngx_cycly_t中的listening动态数组中，每一个监听对象ngx_listening_t对应的读事件中的accept标志位才会是1
    unsigned         accept:1;
...
typedef struct {
	//添加事件方法，负责把1个感兴趣的事件添加到操作系统提供的事件驱动机制中，这样在事件发生后，将可以在调用下面的process_events时获取这个事件
    ngx_int_t  (*add)(ngx_event_t *ev, ngx_int_t event, ngx_uint_t flags);
	//删除事件方法，将1个已经存在于事件驱动机制中的事件移除
	...
	//向事件驱动机制中添加一个新的连接，意味着连接上的读写事件都添加到事件驱动机制中了
	...
	//在正常的工作循环中，将通过调用process_events方法来处理事件。它是处理、分发事件的核心
...
typedef struct {
	...
	//选用的事件模块在所有事件模块中的序号，也即是ctx_index成员
    ngx_uint_t    use;
	//若为1,则表示接收到一个新连接事件时，一次性建立尽可能多的连接
...
#if (NGX_DEBUG)
	//在with-debug编译模式下，可以仅对某些客户端建立的连接输出调试级别的日志，而debug_connection数组用于保存这些客户端的地址信息
...
typedef struct {
	//事件模块的名称
    ngx_str_t              *name;
	//在解析配置项前，这个回调方法用于创建存储配置贡参数的结构体
    void                 *(*create_conf)(ngx_cycle_t *cycle);
	//在解析配置项完成后，init_conf方法会被调用，用以综合处理当前事件模块感兴趣的全部配置项
    char                 *(*init_conf)(ngx_cycle_t *cycle, void *conf);
	//对于事件驱动机制，每个事件模块需要实现的10个抽象方法，此成员是定义事件驱动模块的核心方法
