...
typedef enum {
	...
	//根据URI寻找匹配的location,这个阶段通常由ngx_http_core_module模块实现，不建立其他http模块重新定义这一阶段的行为
    NGX_HTTP_FIND_CONFIG_PHASE,
	//在NGX_HTTP_FIND_CONFIG_PHASE阶段后重写url的意义与NGX_HTTP_SERVER_REWRITE_PHASE阶段显然是不同的，因为这两者会导致查找到不同的location块(location是与URI进行匹配的)
    NGX_HTTP_REWRITE_PHASE,
	//这一阶段是用于在rewrite重写URL后重新跳到NGX_HTTP_FIND_PHASE阶段，找到与新的URI匹配的location.所以，这了阶段是无法由第三方http模块处理的，而仅由ngx_http_core_module模块使用
    NGX_HTTP_POST_REWRITE_PHASE,
	//处理NGX_HTTP_ACCESS_PHASE阶段前，http模块可以介入的处理阶段
    NGX_HTTP_PREACCESS_PHASE,
	//这个阶段用于让http模块判断是否允许这个请求访问nginx服务器
    NGX_HTTP_ACCESS_PHASE,
	//当NGX_HTTP_ACCESS_PHASE阶段中http模块的handler处理方法返回不允许访问的错误码时，这个阶段将负责构造拒绝服务的用户响应。所以，这个阶段实际上用于给NGX_HTTP_ACCESS_PHASE阶段收尾
	...
	//处理完请求后记录日志的阶段。例如，ngx_http_log_module模块不在这个阶段中加入了一个handler处理方法，使得每个http请求处理完毕后会记录access_log日志
    NGX_HTTP_LOG_PHASE
} ngx_http_phases;
