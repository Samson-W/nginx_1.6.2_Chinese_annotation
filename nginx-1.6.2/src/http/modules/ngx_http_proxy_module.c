...
//这个结构主要用于定义各个Hook函数，此结构中的回调接口都是在ngx_http框架初始化完成的工作在这里进行注册
static ngx_http_module_t  ngx_http_proxy_module_ctx = {
	...
	//由于我们的模块仅仅用于location域，所以其它的不关注则设置为NULL
	//ngx_http_proxy_create_loc_conf用于初始化一个配置结构体，如为配置结构体分配内存等工作
    ngx_http_proxy_create_loc_conf,        /* create location configuration */
	//ngx_http_proxy_merge_loc_conf用于将其父block的配置信息合并到此结构体中，也就是实现配置的继承。这两个函数会被Nginx自动调用
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    u->accel = 1;
	//启动upstream机制: 表示读取完用户请求的http包体后才会调用upstream_init启动upstream机制
...
//创建ngx_http_proxy_loc_conf_t结构，分配内存，并初始化数据，返回此结构的指针
static void *
